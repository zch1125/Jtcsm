...
### 4.2 从 PDF 导入完整菜谱库（可选）

项目还提供由公开家常菜 PDF 生成的 579 道菜谱种子数据：
...
`04_seed_recipes_from_pdf.sql` 会覆盖 `recipe` / `recipe_ingredient` / `recipe_step` 的预置数据；`05_recipe_cover_images.sql` 按菜名回填 `/static/recipes/**` 封面图。数据来源与导出说明见 `docs/recipes/README.md`。
